...
Required for Google Drive case-study request uploads:
...
The OAuth token should be generated by signing in as the Google account that
should own request uploads, usually `rfr.rag.uwaterloo@gmail.com`.

Generate the Hugging Face OAuth secrets locally:
...
The older fallback `GOOGLE_SERVICE_ACCOUNT_JSON` service-account setup can fail
in regular My Drive folders because service accounts do not have storage quota.
Use OAuth or a Google Shared Drive for request uploads.
...
- `GOOGLE_OAUTH_CLIENT_ID`, `GOOGLE_OAUTH_CLIENT_SECRET`, and
  `GOOGLE_OAUTH_REFRESH_TOKEN` exist as Hugging Face secrets
- the Drive folder is owned by or shared with the OAuth Google account
- `RFR_REQUESTS_DRIVE_FOLDER_ID` points to the correct folder
- the Space has restarted after secrets or variables were added
