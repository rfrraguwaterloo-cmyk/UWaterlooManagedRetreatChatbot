...
- The three OAuth secrets exist in Hugging Face.
- The refresh token was generated by signing in as the Google account that
  should own uploaded request folders, usually `rfr.rag.uwaterloo@gmail.com`.
- The intake folder is owned by or shared with that Google account.
...
If the error says service accounts do not have storage quota, the app is falling
back to `GOOGLE_SERVICE_ACCOUNT_JSON`. Add the OAuth secrets above, or move the
